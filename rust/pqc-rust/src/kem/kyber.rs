        ...
        let (ct_bytes, ss_bytes) = match self.algorithm {
            KemAlgorithm::Kyber512 => {
                let pk = pqcrypto_kyber::kyber512::PublicKey::from_bytes(public_key.as_bytes())
                    .map_err(|e| {
                        PqcError::SerializationError(format!("Invalid public key bytes: {:?}", e))
                    })?;
        ...
        let ss_bytes = match self.algorithm {
            KemAlgorithm::Kyber512 => {
                let ct = pqcrypto_kyber::kyber512::Ciphertext::from_bytes(ciphertext.as_bytes())
                    .map_err(|e| {
                        PqcError::SerializationError(format!("Invalid ciphertext bytes: {:?}", e))
                    })?;
                let sk =
                    pqcrypto_kyber::kyber512::SecretKey::from_bytes(secret_key.expose_secret())
                        .map_err(|e| {
                            PqcError::SerializationError(format!(
            ...
            KemAlgorithm::Kyber768 => {
                ...
                let sk =
                    pqcrypto_kyber::kyber768::SecretKey::from_bytes(secret_key.expose_secret())
                        .map_err(|e| {
                            PqcError::SerializationError(format!(
                                "Invalid secret key bytes: {:?}",
                                e
            ...
            KemAlgorithm::Kyber1024 => {
                ...
                let sk =
                    pqcrypto_kyber::kyber1024::SecretKey::from_bytes(secret_key.expose_secret())
                        .map_err(|e| {
                            PqcError::SerializationError(format!(
                                "Invalid secret key bytes: {:?}",
                                e
