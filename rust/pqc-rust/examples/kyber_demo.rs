use pqc_rust::{Kyber, KemAlgorithm, Kem, PqcError};
use rand_core::OsRng;
...
fn main() -> Result<(), PqcError> {
    ...
    println!(" -> Bob's Public Key length: {} bytes", public_key.as_bytes().len());
    println!(" -> Bob's Secret Key length: {} bytes", secret_key.expose_secret().len());
    ...
    println!(" -> Ciphertext length: {} bytes", ciphertext.as_bytes().len());
    println!(" -> Alice's Derived Secret: {:x?}...", &alice_secret.expose_secret()[0..12]);

    // 4. Decapsulation (Bob)
    println!("\n[Step 3] Decapsulating ciphertext using Bob's Secret Key (Bob)...");
    let bob_secret = kyber.decapsulate(&ciphertext, &secret_key)?;
    println!(" -> Bob's Derived Secret:   {:x?}...", &bob_secret.expose_secret()[0..12]);
    ...
    if alice_secret.expose_secret() == bob_secret.expose_secret() {
        ...
        return Err(PqcError::DecapsulationError("Shared secret mismatch".to_string()));
    }
    println!("==================================================");
